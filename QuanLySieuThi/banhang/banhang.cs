...
namespace QuanLySieuThi.banhang
{
    public partial class banhang : Form
    {
        ...
        {
            db1.CellClick += db1_CellClick;

        }

        public  void Load_dta1()
        {
            chuoiketnoi.Chuoiketnoi_Data(chuoi, db1);
        ...
        private void db1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txt_mathuoc.Text = row.Cells["MaSP"].Value.ToString();
                txt_tenthuoc.Text = row.Cells["TenSP"].Value.ToString();
                txt_dangthuoc.Text = row.Cells["DonViTinh"].Value.ToString();
                txt_thuoctrongkho.Text = row.Cells["Solg"].Value.ToString();
